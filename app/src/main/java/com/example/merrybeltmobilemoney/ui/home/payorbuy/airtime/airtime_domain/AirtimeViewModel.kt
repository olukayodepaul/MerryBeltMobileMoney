...
class AirtimeViewModel @Inject constructor( //this are the use cases
    ...
    private fun onAirtimeProductSelected(
        airtimeProductSelected: String,
        airtimeProductImage: String,
        airtimeProductCategory: String
    ) {
        uiState.value = uiState.value.copy(
    ...
    fun airtimeEventHandler(airtimeEvent: AirtimeEvent) {
        when (airtimeEvent) {
            ...
            is AirtimeEvent.OnAirtimeProductSelected -> {
                onAirtimeProductSelected(
                    airtimeEvent.airtimeProductSelected,
                    airtimeEvent.airtimeProductImage,
                    airtimeEvent.airtimeProductCategory
                )
            }
    ...
    init {
        viewModelScope.launch {
            try {
                val billProduct = repo.getBillingProduct(
                    repo.customerProfile().terminalId,
                    repo.customerProfile().sessionId,
                    "airtime"
                )
                val decryptedAirtime = EncryptionUtil().isDecryption(
                    billProduct.body()!!.data,
                    repo.customerProfile().sessionId
                )
                val getAirtimeListOfVariousNetwork: List<AirtimeProductList> =
                    Constant.gson.fromJson(decryptedAirtime, Array<AirtimeProductList>::class.java)
                        .toList()
